<%@ Page Title="" Language="VB" MasterPageFile="~/MasterPage.master" AutoEventWireup="false" CodeFile="Withdraw.aspx.vb" Inherits="Teller_Withdraw" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    ...
    <script>
        function limitInputs() {
            $("#<%= WithdrawAccountNumber.ClientID %>").on("input", function () {
                if (this.value.length > 10) {
                    this.value = this.value.substring(0, 10);
                }
            });

            $("#<%= WithdrawAmount.ClientID %>").on("input", function () {
                    if (this.value.length > 8) {
                        this.value = this.value.substring(0, 8);
                    }
                });

            $("#<%= WithdrawAmount.ClientID %>").on("blur", function () {
...
<asp:Content ID="Content2" ContentPlaceHolderID="Navbar" Runat="Server">
    <ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
        ...
        <li class="nav-item me-0 me-lg-3">
            <a class="nav-link ms-3 ms-lg-0 active" href="./Withdraw.aspx">Withdraw</a>
        </li>
        <li class="nav-item me-0 me-lg-3">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-12 col-sm-6 mb-3 mb-sm-0">
                            <div class="form-floating">
                                <asp:TextBox class="form-control" required placeholder="Account Number (Ex: 7810250000)" type="number" ID="WithdrawAccountNumber" runat="server"></asp:TextBox>
                                <label for="<%= WithdrawAccountNumber.ClientID %>">Account Number (Ex: 7810250000)</label>
                            </div>
                        </div>

                        <div class="col-12 col-sm-6">
                            <div class="form-floating">
                                <asp:TextBox class="form-control" required placeholder="Amount (Ex: 100.00)" type="number" ID="WithdrawAmount" runat="server"></asp:TextBox>
                                <label for="<%= WithdrawAmount.ClientID %>">Amount (Ex: 100.00)</label>
                            </div>
                        </div>
                    </div>
