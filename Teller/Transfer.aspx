<%@ Page Title="" Language="VB" MasterPageFile="~/MasterPage.master" AutoEventWireup="false" CodeFile="Transfer.aspx.vb" Inherits="Teller_Transfer" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    ...
    <script>
        function limitInputs() {
            $("#<%= TransferAccountNumberSender.ClientID %>, #<%= TransferAccountNumberReceiver.ClientID %>").on("input", function () {
                if (this.value.length > 10) {
                    this.value = this.value.substring(0, 10);
                }
            });

            $("#<%= TransferAmount.ClientID %>").on("input", function () {
                    if (this.value.length > 8) {
                        this.value = this.value.substring(0, 8);
                    }
                });

            $("#<%= TransferAmount.ClientID %>").on("blur", function () {
                    ...
                    <div class="row justify-content-center">
                        <div class="col-12 col-sm-6 mb-3 mb-sm-0">
                            <div class="form-floating">
                                <asp:TextBox class="form-control" required placeholder="Sender Account Number (Ex: 7810250000)" type="number" ID="TransferAccountNumberSender" runat="server"></asp:TextBox>
                                <label for="<%= TransferAccountNumberSender.ClientID %>">Sender Account Number (Ex: 7810250000)</label>
                            </div>
                        </div>

                        <div class="col-12 col-sm-6 mb-3 mb-sm-0">
                            <div class="form-floating">
                                <asp:TextBox class="form-control" required placeholder="Receiver Account Number (Ex: 7810250000)" type="number" ID="TransferAccountNumberReceiver" runat="server"></asp:TextBox>
                                <label for="<%= TransferAccountNumberReceiver.ClientID %>">Receiver Account Number (Ex: 7810250000)</label>
                            </div>
                        </div>

                        <div class="col-12 col-sm-6 mb-3 mb-sm-0 mt-sm-3">
                            <div class="form-floating">
                                <asp:TextBox class="form-control" required placeholder="Amount (Ex: 100.00)" type="number" ID="TransferAmount" runat="server"></asp:TextBox>
                                <label for="<%= TransferAmount.ClientID %>">Amount (Ex: 100.00)</label>
                            </div>
                        </div>
                    </div>

                    <div class="row justify-content-center mt-0 mt-sm-3">
